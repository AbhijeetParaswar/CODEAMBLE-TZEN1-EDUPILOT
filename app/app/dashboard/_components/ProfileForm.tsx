...
import { updateProfile, type StudentProfile } from "@/lib/api/api";
...
export default function ProfileForm({
  ...
  function addPrefTag(key: string, value: string) {
    ...
    if (!current.includes(value.trim()))
      setPref(key, [...current, value.trim()]);
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-w-2xl">
      ...
      {message && (
        <p
          className={`font-mono text-[12px] transition-all duration-500 px-4 py-2 border flex items-center gap-2 ${
            message.ok
      ...
      <Section title="Personal Details">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 transition-all duration-500">
          ...
          <Field
            ...
            onChange={(v) => set("income_annual", Number(v))}
          />
        </div>
      </Section>
      ...
      <Section title="Academic Info">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 transition-all duration-500">
          ...
          <Field
            ...
            onChange={(v) => set("year_of_study", Number(v))}
          />
          <Field
            label="CGPA"
            type="number"
            value={form.cgpa ?? ""}
            onChange={(v) => set("cgpa", Number(v))}
          />
          <Field
            label="12th %"
            type="number"
            value={form.percentage_12th ?? ""}
            onChange={(v) => set("percentage_12th", Number(v))}
          />
          <Field
...
function Section({
  ...
  return (
    <section className="border border-black/10 dark:border-white/8 p-5 flex flex-col gap-4 transition-all duration-500">
      <h2 className="font-syne font-bold text-sm text-gray-900 dark:text-[#F0F4FF] transition-all duration-500">
        {title}
      </h2>
      {children}
    </section>
...
function Field({
  ...
  return (
    <label className="flex flex-col gap-1 transition-all duration-500">
      ...
      />
      {hint && (
        <span className="font-mono text-[10px] text-gray-300 dark:text-gray-600 transition-all duration-500">
          {hint}
        </span>
      )}
    </label>
  ...
  return (
    <div className="flex flex-col gap-1 transition-all duration-500">
      <span className="font-mono text-[11px] text-gray-400 dark:text-[#6B7280] transition-all duration-500">
        {label}
        {optional && (
          <span className="ml-1 text-gray-300 dark:text-gray-600 transition-all duration-500">
            (optional)
          </span>
        )}
      </span>
      ...
      <div className="flex items-center gap-3 px-3 py-2 border border-black/10 dark:border-white/8 bg-white dark:bg-[#161822] transition-all duration-500">
        ...
        <span className="flex-1 font-mono text-[12px] text-gray-500 truncate">
          {fileName || (
            <span className="text-gray-300 dark:text-gray-600">{hint}</span>
          )}
        </span>
        <button
          type="button"
