...
import type { InternshipMatchResult } from "@/lib/api/api";
import { saveInternship } from "@/lib/api/api";
...
const PLATFORMS: Record<string, { label: string; color: string; bg: string }> =
  {
    internshala: {
      label: "Internshala",
      color: "text-[#007bff]",
      bg: "bg-[#007bff]/8 border-[#007bff]/20",
    },
    indeed: {
      label: "Indeed",
      color: "text-[#2557a7]",
      bg: "bg-[#2557a7]/8 border-[#2557a7]/20",
    },
    naukri: {
      label: "Naukri",
      color: "text-[#ff7555]",
      bg: "bg-[#ff7555]/8 border-[#ff7555]/20",
    },
    wellfound: {
      label: "Wellfound",
      color: "text-[#ef4444]",
    ...
    aicte: {
      label: "AICTE",
      color: "text-[#16a34a]",
      bg: "bg-[#16a34a]/8 border-[#16a34a]/20",
    },
  };
...
export default function InternshipsClient({ userId, userEmail, token }: Props) {
  ...
  const doSearch = useCallback(
    async (q: string, f: Filters, isInitial = false) => {
      setLoading(true);
  ...
  const activePills = [
    filters.work_type
      ? {
          key: "work_type" as keyof Filters,
          label:
    ...
    filters.duration_max
      ? {
          key: "duration_max" as keyof Filters,
          label: `\u2264 ${filters.duration_max}m`,
        }
      : null,
    filters.stipend_min
      ? {
          key: "stipend_min" as keyof Filters,
          label: `\u20b9${Number(filters.stipend_min).toLocaleString()}+/mo`,
        }
      : null,
    filters.location
    ...
    <div className="flex flex-col gap-5 transition-all duration-500">
      <div className="transition-all duration-500">
        ...
        <p className="font-mono text-[12px] text-gray-400 dark:text-[#6B7280] mt-1">
          Matched from Internshala, Indeed, Naukri, Wellfound &amp; Unstop
          &middot; ranked by your profile fit
        </p>
      </div>
      <form
      ...
      >
        <div className="relative flex-1">
          <Search
            size={14}
            className="absolute transition-all duration-500 left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
          />
          <input
            value={query}
        ...
        <div className="border border-black/10 dark:border-white/8 bg-gray-50 dark:bg-[#0F1117] p-4 grid grid-cols-2 md:grid-cols-5 gap-4 transition-all duration-500">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1">
              {(
                [
                  { v: "remote", I: Wifi, l: "Remote / WFH" },
                  { v: "hybrid", I: GitMerge, l: "Hybrid" },
                  { v: "onsite", I: Building2, l: "On-site" },
                ] as const
              ).map(({ v, I, l }) => (
                <button
                  key={v}
          ...
          <div className="flex flex-col gap-1.5 transition-all duration-500">
            ...
            <div className="flex flex-col gap-1">
              {[
                { v: "1", l: "Up to 1 month" },
                { v: "2", l: "Up to 2 months" },
                { v: "3", l: "Up to 3 months" },
                { v: "6", l: "Up to 6 months" },
              ].map(({ v, l }) => (
                <button
                  key={v}
                  type="button"
      ...
      {!searched && matches.length > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 border border-[#0C65D2]/20 bg-[#0C65D2]/5 font-mono text-[11px] text-[#0C65D2] transition-all duration-500">
          <User size={12} /> Showing recommendations personalised to your
          profile. Use search or filters to refine.
        </div>
      )}
...
function InternshipCard({
  ...
  const stipend =
    o.amount_min && o.amount_max
      ...
      : o.amount_max
        ? `Up to \u20b9${o.amount_max.toLocaleString()}/mo`
        : o.amount_min
    ...
    <div className="border border-black/10 dark:border-white/8 bg-white dark:bg-[#0F1117] flex flex-col transition-all duration-500">
      ...
      <div className="px-4 pb-2 flex items-center gap-2 border-t border-black/5 dark:border-white/5 pt-2 transition-all duration-500">
        ...
        <div className="flex-1 h-1 bg-gray-100 dark:bg-white/6 overflow-hidden">
          <div
            className={`h-full ${match_score >= 70 ? "bg-green-500" : match_score >= 45 ? "bg-[#0C65D2]" : "bg-gray-300"}`}
      ...
      {expanded && (
        <div className="px-4 pb-4 flex flex-col gap-3 border-t border-black/5 dark:border-white/5 pt-3 transition-all duration-500">
          {reasons.length > 0 && (
            <div>
              <p className="font-mono text-[10px] text-gray-400 uppercase tracking-wide mb-1">
                Match breakdown
              </p>
