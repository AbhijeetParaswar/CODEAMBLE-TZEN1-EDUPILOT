...
from eval.judge_config import get_deepeval_judge_llm
...
def _faithfulness():
    return FaithfulnessMetric(threshold=THRESHOLD, model=get_deepeval_judge_llm(), verbose_mode=False)


def _answer_relevancy():
    return AnswerRelevancyMetric(threshold=THRESHOLD, model=get_deepeval_judge_llm(), verbose_mode=False)


def _context_precision():
    return ContextualPrecisionMetric(threshold=THRESHOLD, model=get_deepeval_judge_llm(), verbose_mode=False)
