...
def evaluate_eligibility(profile: StudentProfile | None, rules: dict) -> dict:
    ...
    # ── general minimum percentage (10th or previous year) ────────────────
    if min_pct_gen := rules.get("min_percentage"):
        # Use best available marks: previous_year > 12th > 10th
        best_pct = (
            getattr(profile, "previous_year_marks_percentage", None)
            or profile.percentage_12th
            or getattr(profile, "percentage_10th", None)
        )
        if best_pct is not None:
            if best_pct >= min_pct_gen:
                passed.append(f"Marks {best_pct}% meets minimum {min_pct_gen}%")
                score_factors.append(1.0)
            else:
                failed.append(f"Marks {best_pct}% below minimum {min_pct_gen}%")
        else:
            warnings.append(f"Academic marks not provided (minimum {min_pct_gen}% required)")

    # ── academic level ────────────────────────────────────────────────────
    if academic_levels := rules.get("academic_levels"):
        student_level = getattr(profile, "academic_level", None)
        if student_level:
            # Fuzzy match: "Undergraduate (UG)" matches rule value "UG"
            al_lower = student_level.lower()
            if any(lvl.lower() in al_lower or al_lower in lvl.lower() for lvl in academic_levels):
                passed.append(f"Academic level {student_level} eligible")
                score_factors.append(1.0)
            else:
                failed.append(f"Academic level must be: {', '.join(academic_levels)}")
        else:
            warnings.append("Academic level not set in profile")

    # ── institution type ─────────────────────────────────────────────────
    if inst_types := rules.get("institution_type"):
        student_inst = getattr(profile, "institution_type", None)
        if student_inst:
            if any(it.lower() in student_inst.lower() or student_inst.lower() in it.lower() for it in inst_types):
                passed.append(f"Institution type {student_inst} eligible")
                score_factors.append(1.0)
            else:
                failed.append(f"Institution type must be: {', '.join(inst_types)}")
        else:
            warnings.append("Institution type not set in profile")

    # ── PwD requirement ───────────────────────────────────────────────────
    if rules.get("is_pwd_required"):
        is_pwd = getattr(profile, "is_pwd", None)
        if is_pwd:
            min_pwd_pct = rules.get("min_pwd_percentage", 0)
            pwd_pct = getattr(profile, "pwd_percentage", None) or 0
            if pwd_pct >= min_pwd_pct:
                passed.append(f"PwD status verified ({pwd_pct}% disability)")
                score_factors.append(1.0)
            else:
                failed.append(f"Minimum {min_pwd_pct}% disability required for this scheme")
        else:
            failed.append("This scheme is exclusively for Persons with Disability (PwD)")

    # ── Single girl child ─────────────────────────────────────────────────
    if rules.get("is_single_girl_child_eligible"):
        if getattr(profile, "is_single_girl_child", None):
            passed.append("Single girl child — additional benefit applicable")
            score_factors.append(1.0)
        # Not a hard fail — scheme may be open to all girls but grants extra benefit

    # ── Orphan eligibility ────────────────────────────────────────────────
    if rules.get("is_orphan_eligible"):
        is_orphan = getattr(profile, "is_orphan", None)
        is_ward   = getattr(profile, "is_ward_of_defense_personnel", None)
        if is_orphan or is_ward:
            label = "Orphan" if is_orphan else "Ward of defence/Covid victim"
            passed.append(f"{label} — eligible for Swanath scheme")
            score_factors.append(1.0)
        else:
            failed.append("This scheme requires orphan or defence/Covid ward status")

    # ── Defence ward eligibility ──────────────────────────────────────────
    if rules.get("is_defense_ward_eligible") and not rules.get("is_orphan_eligible"):
        # Only check separately if not already covered by is_orphan_eligible above
        if getattr(profile, "is_ward_of_defense_personnel", None):
            passed.append("Ward of defence personnel — eligible")
            score_factors.append(1.0)
    ...
    if year_range := rules.get("year_of_study"):
        if profile.year_of_study is not None:
            # Support both dict {min, max} and list [1, 2, 3] formats
            if isinstance(year_range, list):
                if profile.year_of_study in year_range:
                    passed.append(f"Year {profile.year_of_study} eligible")
                    score_factors.append(1.0)
                else:
                    failed.append(f"Year of study must be one of: {year_range}")
            else:
